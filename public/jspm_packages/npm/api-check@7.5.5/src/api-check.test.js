...
describe('apiCheck', () => {
  ...
  describe(`main export`, () => {
    ...
    it(`should allow you to create instances of apiCheck that do not conflict`, () => {
      const apiCheck1 = getApiCheck({output: {prefix: 'apiCheck1'}});
      const apiCheck2 = getApiCheck({output: {prefix: 'apiCheck2'}});
    ...
    it(`should throw an error when the config passed is improperly shaped`, () => {
      expect(() => getApiCheck({prefix: 'apiCheck1'})).to.throw(makeSpacedRegex('creating an apiCheck instance apiCheck failed! prefix apiCheck1'));
    });
    it(`should throw an error when the checkers passed are improperly shaped`, () => {
      const myImproperChecker = coveredFunction();
      myImproperChecker.type = false;
      expect(() => getApiCheck(null, {myChecker: myImproperChecker})).to.throw(makeSpacedRegex('creating an apiCheck instance apiCheck failed! myChecker'));
    });
    it(`should allow for specifying only default config`, () => {
      const docsBaseUrl = 'http://my.example.com';
      const apiCheck1 = getApiCheck({output: {docsBaseUrl}});
    ...
    it(`should allow for specifying both extra checkers and default config`, () => {
      const docsBaseUrl = 'http://my.example.com';
      const apiCheck1 = getApiCheck({output: {docsBaseUrl}}, {myChecker: coveredFunction});
      expect(apiCheck1.config.output.docsBaseUrl).to.equal(docsBaseUrl);
  ...
  describe('#', () => {
    ...
    it(`should handle the crazy optional specifications`, () => {
      function crazyFunction() {
        const message = apiCheckInstance([apiCheckInstance.string.optional, apiCheckInstance.number.optional, apiCheckInstance.bool, apiCheckInstance.object.optional, apiCheckInstance.func.optional, apiCheckInstance.array, apiCheckInstance.string.optional, apiCheckInstance.func], arguments).message;
        expect(message).to.be.empty;
      }
    ...
    it(`should handle a final two optional arguments`, () => {
      (function(a, b, c) {
        const message = apiCheckInstance([apiCheckInstance.string, apiCheckInstance.oneOfType([apiCheckInstance.arrayOf(apiCheckInstance.string), apiCheckInstance.shape({name: apiCheckInstance.string})]).optional, apiCheckInstance.shape({
    ...
    it(`should show the user what they provided in a good way`, () => {
      (function(a, b, c) {
        ...
        expect(result.message).to.match(makeSpacedRegex('you passed coveredFunction false anonymous function types function boolean function'));
    ...
    describe(`api checking`, () => {
      ...
      it(`should throw an error when a checker is specified with the incorrect shape`, () => {
        ipAddressChecker.type = {__apiCheckData: {
            type: 'ipAddress',
            optional: false
          }};
        expect(() => apiCheckInstance(ipAddressChecker, args)).to.throw();
      ...
      it(`should throw an error when specifying the api as an array, but the args is not an array`, () => {
        const api = [apiCheckInstance.string, apiCheckInstance.number];
        expect(() => apiCheckInstance(api, 'foo')).to.throw(makeSpacedRegex('if array api array args you passed "foo" with the type: string'));
      });
    });
  ...
  describe('#disable/enable', () => {
    ...
    it(`should use the noop version of checkers when initializing a new instance if globally disabled`, () => {
      ...
      const checkers = [customInstance.string, customInstance.bool, customInstance.func, customInstance.array, customInstance.number, customInstance.object, customInstance.object.nullOk, customInstance.oneOf([null, 'foo']), customInstance.oneOfType([customInstance.string.optional, customInstance.bool.optional]), customInstance.arrayOf(customInstance.string), customInstance.objectOf(customInstance.array), customInstance.instanceOf(Date), customInstance.shape({}), customInstance.shape.ifNot('foo'), customInstance.shape.onlyIf(['bar', 'baz']), customInstance.typeOrArrayOf(customInstance.string), customInstance.args, customInstance.any];
  ...
  describe('#getErrorMessage', () => {
    ...
    it(`should show the user's arguments and types nicely`, () => {
      const result = apiCheckInstance.getErrorMessage([apiCheckInstance.object, apiCheckInstance.array.optional, apiCheckInstance.string], [{
        a: 'a',
        r: new RegExp(),
        b: undefined
      }, [23, false, null]]);
      const regex = makeSpacedRegex('you passed a a r {} [ 23 false null ] with the types: a string r regexp b undefined number boolean null ' + 'the api calls for object array \\(optional\\) string');
    ...
    it('should be overrideable', () => {
      ...
      apiCheckInstance.getErrorMessage = (_api, _args, _message, _output) => {
        ...
        expect(_args).to.eql(Array.prototype.slice.call(args));
      ...
      const terseMessage = {
        ...
        shape: {
          ...
          bar: {
            ...
            'func.withProperties': {bar2: 'Boolean <-- YOU ARE MISSING THIS'}
          }
        }
      };
