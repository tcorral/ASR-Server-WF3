...
describe('checkers', () => {
    ...
    describe(`function`, () => {
      ...
      describe(`.withProperties`, () => {
        it(`should check for properties on a function`, () => {
          ...
          myFuncWithProps.anotherProp = {anotherFunction: anotherFunctionWithProps};
          const checker = checkers.func.withProperties({
            someProp: checkers.string,
            anotherProp: checkers.shape({anotherFunction: checkers.func.withProperties({aNumber: checkers.number})})
          });
  ...
  describe('oneOf', () => {
    ...
    it(`should work with typeOrArrayOf and null`, () => {
      expect((checkers.oneOfType([checkers.oneOf([null, 'ehy', {a: 'b'}, undefined]), checkers.typeOrArrayOf(checkers.string)]))(null)).to.be.undefined;
    });
  });
  describe('oneOfType', () => {
    ...
    it(`should have the full checker type of its children`, () => {
      const checker = checkers.oneOfType([checkers.shape({
        name: checkers.string,
        value: checkers.oneOfType([checkers.string, checkers.arrayOf(checkers.number).optional]).optional
      }), checkers.func]);
  ...
  describe('objectOf', () => {
    it('should pass when the object contains only properties of a type of the type given', () => {
      expect(checkers.objectOf(checkers.bool)({
        a: true,
        b: false,
        c: true
      })).to.be.undefined;
    ...
    it('should fail when one of the properties does not match the type', () => {
      expect(checkers.objectOf(checkers.number)({
        a: 1,
        b: 'string',
        c: 3
  ...
  describe('shape', () => {
    ...
    it('should fail when the object is missing any of the properties specified', () => {
      const check = checkers.shape({scores: checkers.objectOf(checkers.number)});
    ...
    it('should have an optional function that does the same thing', () => {
      const check = checkers.shape({appliances: checkers.arrayOf(checkers.object)}).optional;
      expect(check({appliances: [{name: 'refridgerator'}]})).to.be.undefined;
    });
    it('should be false when passed a non-object', () => {
      const check = checkers.shape({friends: checkers.arrayOf(checkers.object)});
    ...
    it(`should display the location of sub-children well`, () => {
      const obj = {person: {home: {location: {street: 324}}}};
      const check = checkers.shape({person: checkers.shape({home: checkers.shape({location: checkers.shape({street: checkers.string})})})});
      ...
      expect(typeTypes).to.eql({
        ...
        candy: {
          __apiCheckData: {
            type: 'arrayOf',
            optional: false,
            error: 'THIS IS THE PROBLEM: `candy` must be `arrayOf[shape]`'
          },
          arrayOf: {
            __apiCheckData: {
              strict: false,
            ...
            shape: {good: 'Boolean <-- YOU ARE MISSING THIS'}
          }
        }
      });
    });
    it(`should handle a checker with no type and still look ok`, () => {
      const check = checkers.shape({voyager: checkers.shape({seasons: coveredFunction})});
      const obj = {voyager: {seasons: 7}};
      const typeTypes = check.type({
        terse: true,
        obj,
        addHelpers: true
      });
      expect(typeTypes).to.eql({voyager: {
          __apiCheckData: {
            type: 'shape',
            strict: false,
            optional: false
          },
          shape: {seasons: 'coveredFunction'}
        }});
    });
    it(`should handle a checker with no type and not break when there's a failure`, () => {
      const check = checkers.shape({voyager: checkers.shape({seasons: coveredFunction})});
      const obj = {voyager: 'failure!?'};
      const typeTypes = check.type({
        terse: true,
        obj,
        addHelpers: true
      ...
      expect(typeTypes).to.eql({voyager: {
          ...
          shape: {seasons: 'coveredFunction <-- YOU ARE MISSING THIS'}
        }});
    });
    ...
    describe('ifNot', () => {
      ...
      it('should pass if the specified array of properties do not exist', () => {
        const check = checkers.shape({cookies: checkers.shape.ifNot(['mint', 'chips'], checkers.bool)});
        expect(check({cookies: true})).to.be.undefined;
      });
      ...
      it('should fail if it fails the specified checker', () => {
        const check = checkers.shape({cookies: checkers.shape.ifNot('mint', checkers.bool)});
      ...
      it(`should have a legible type`, () => {
        const check = checkers.shape({
          ...
          childrenNames: checkers.arrayOf(checkers.string),
          optionalStrictObject: checkers.shape({somethingElse: checkers.objectOf(checkers.shape({prop: checkers.func}).optional)}).strict.optional
        });
    ...
    describe('onlyIf', () => {
      it('should pass only if the specified property is also present', () => {
        const check = checkers.shape({cookies: checkers.shape.onlyIf('mint', checkers.bool)});
        expect(check({
          cookies: true,
          mint: true
        })).to.be.undefined;
      ...
      it('should pass only if all specified properties are also present', () => {
        const check = checkers.shape({cookies: checkers.shape.onlyIf(['mint', 'chip'], checkers.bool)});
        expect(check({
          cookies: true,
          mint: true,
          chip: true
      ...
      it(`should not throw an error if you specify onlyIf with a checker`, () => {
        ...
        const shapeChecker = checkers.func.withProperties({type: checkers.oneOfType([checkers.func.withProperties({__apiCheckData: __apiCheckDataChecker}), checkers.shape({__apiCheckData: __apiCheckDataChecker})])});
        const check = checkers.shape({
          oneChecker: checkers.shape.onlyIf('otherChecker', shapeChecker).optional,
          otherChecker: shapeChecker.optional
        });
        const invalidValue = {oneChecker: checkers.shape({})};
        expect(() => {
    ...
    describe(`requiredIfNot`, () => {
      ...
      describe(`all`, () => {
        ...
        it(`should throw an error when trying to create an all with anything but an array`, () => {
          expect(() => checkers.shape.requiredIfNot.all('hi', checkers.bool)).to.throw('requiredIfNot.all must be passed an array');
        });
      });
    });
  });
  ...
  describe(`all checkers`, () => {
    const builtInCheckers = [checkers.array, checkers.bool, checkers.number, checkers.string, checkers.func, checkers.object, checkers.instanceOf(Date), checkers.oneOf([null]), checkers.oneOfType([checkers.bool]), checkers.arrayOf(checkers.string), checkers.objectOf(checkers.func), checkers.typeOrArrayOf(checkers.number), checkers.shape({}), checkers.args, checkers.any, checkers.null, checkers.range(1, 15), checkers.lessThan(10), checkers.greaterThan(15), checkers.emptyObject];
